...
namespace Handyman.Azure.Functions.Http.ApiVersioning
{
    internal class SemanticVersionComparer : IComparer<SemanticVersion>
    {
        ...
        public int Compare(SemanticVersion x, SemanticVersion y)
        {
            ...
            var xSegments = x.PreRelease.Length == 0
                ? EmptySegments
                : x.PreRelease.Split(new[] {"."}, StringSplitOptions.None);
            var ySegments = y.PreRelease.Length == 0
                ? EmptySegments
                : y.PreRelease.Split(new[] {"."}, StringSplitOptions.None);
